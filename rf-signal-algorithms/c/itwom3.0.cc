...
double saalos(double d, PropType & prop, PropAType & propa)
{
	...
	if (d == 0.0) {
		...
		if (prop.tgh > prop.cch) {	/* for TX ant above all clutter height */
			...
			arte =
			    arte -
			    (mymax(0.01, log10(prop.wn * 47.7) - 2)) *
			    (prop.hg[1] / prop.tgh);
		}
		saalosv = arte;
	}
...
double adiff(double d, PropType & prop, PropAType & propa)
{
	...
	if (d == 0) {
		...
		q = (wd1 +
		     xd1 / d) * mymin(((1.0 - 0.8 * exp(-d / 50e3)) * prop.dh *
				       prop.wn), 6283.2);
		wd = 25.1 / (25.1 + sqrt(q));
...
double alos(double d, PropType & prop, PropAType & propa)
{
	...
	if (d == 0.0) {
		wls =
		    0.021 / (0.021 +
			     prop.wn * prop.dh / mymax(10e3, propa.dlsa));
		alosv = 0.0;
	}
	     ...
	     PropAType & propa, PropVType & propv)
{
	...
	if ((np < 1) || (pfl[1] > 150.0)) {
		...
		prop.thenr =
		    atan(mymax(0.0, (pfl[np + 2] - pfl[np + 1])) / pfl[1]);
...
void point_to_pointMDH_two(double *elev, double tht_m, double rht_m, double eps_dielect,
			   double sgm_conductivity, double eno_ns_surfref,
...
void point_to_pointDH(double *elev, double tht_m, double rht_m, double eps_dielect,
		      double sgm_conductivity, double eno_ns_surfref,
		      double enc_ncc_clcref, double clutter_height,
		      double clutter_density, double delta_h_diff,
		      double frq_mhz, int radio_climate, int pol, double conf,
