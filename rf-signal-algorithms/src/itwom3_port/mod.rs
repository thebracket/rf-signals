            ...
            if !wscat {
                ...
                if a5 < 1000.0 {
                    ...
                    propa.dx = mymax(
                        propa.dlsa,
                        mymax(
                            propa.dla + 0.3 * xae * log(47.7 * prop.wn),
...
unsafe fn adiff2(d: f64, prop: &mut PropType, propa: &PropAType) -> f64 {
    ...
    if d == 0.0 {
        ...
        dtr =
            sqrt(prop.dist * prop.dist + (prop.rch[0] - prop.rch[1]) * (prop.rch[0] - prop.rch[1]));
                ...
                if prop.the[1] < 0.2 {
                    ...
                    if prop.hht < 3400.0 {
                        ...
                        if pd >= 3.141592654 {
                            ...
                            csd = abq_alos(
                                Complex::new(sdl, 0.0) + Complex::new(kem * cos(pd), kem * sin(pd)),
                            );
                        }
...
mod test {
    ...
    #[test]
    fn basic_fspl_test() {
        let terrain_path = PTPPath::new(vec![1.0; 200], 100.0, 100.0, 10.0).unwrap();
