...
pub(crate) fn h0f(r: f64, et: f64) -> f64 {
    ...
    if q != 0.0 {
        h0fv = (1.0 - q) * h0fv + q * 4.343 * log((a[it as usize] * x + b[it as usize]) * x + 1.0);
    }
    ...
    if d == 0.0 {
        ...
        if prop.tgh > prop.cch {
            ...
            if ssnps <= 0.0 {
                ...
                if prop.ptx >= 1 {
                    /* polarity ptx is vertical or circular */
                    q = (ensa * cttc - encca * ctic) / (ensa * cttc + encca * ctic);
                    rsp = q * q;
                    tsp = 1.0 - rsp;

                    if prop.ptx == 2 {
                        /* polarity is circular - new */
                    ...
                    /* ptx is 0, horizontal, or undefined */
            ...
            if d1a < 50.0 {
                ...
                if d1a < 225.0 {
                    ...
                    arte = q
                        + (0.7 * pdk - f64::max(0.01, log10(prop.wn * 47.7) - 2.0))
                            * (prop.hg[1] / hone);
                } else {
                    q = 0.00055 * (pdk) + log10(pdk) * (0.041 - 0.0017 * sqrt(hone) + 0.019);

                    arte = d1a * q - (18.0 * log10(rsp)) / (exp(hone / 37.5));
                    ...
                    if pdk > zi {
                        q = (pdk - zi)
                            * 10.2
                            * ((sqrt(f64::max(0.01, log10(prop.wn * 47.7) - 2.0))) / (100.0 - zi));
                    } else {
                        q = ((zi - pdk) / zi)
                            * (-20.0 * f64::max(0.01, log10(prop.wn * 47.7) - 2.0))
                            / sqrt(hone);
                    }
                    arte = arte + q;
                }
            ...
            /* for TX at or below clutter height */

            q = (prop.cch - prop.tgh) * (2.06943 - 1.56184 * exp(1.0 / prop.cch - prop.tgh));
            q = q + (17.98 - 0.84224 * (prop.cch - prop.tgh)) * exp(-0.00000061 * pd);
            arte = q + 1.34795 * 20.0 * log10(pd + 1.0);
            arte = arte - (f64::max(0.01, log10(prop.wn * 47.7) - 2.0)) * (prop.hg[1] / prop.tgh);
        }
        saalosv = arte;
    }
    saalosv
...
pub(crate) fn hzns2(pfl: &[f64], prop: &mut PropType) {
    ...
    if np >= 2 {
        ...
        if !wq {
            ...
            prop.the[0] = ((prop.hht - za) / prop.dl[0]).atan() - 0.5 * prop.gme * prop.dl[0];
            prop.the[1] = ((prop.hhr - zb) / prop.dl[1]).atan() - 0.5 * prop.gme * prop.dl[1];
        }
    }
